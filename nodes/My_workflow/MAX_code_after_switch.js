// MAX code after switch
// Вход: item из ветки channel=max после Expand per store
//
...
const text =
  (src.max?.text ?? src.text ?? '').toString();
...
const normMediaObj = (m) => {
  ...
  const type =
    (m.type ?? m.mediaType ?? '').toString().trim().toLowerCase();

  const url =
    (m.media ?? m.url ?? '').toString().trim();
...
// Дедуп URL (на всякий) + сборка items
const seen = new Set();
const out = [];
let idx = 0;
